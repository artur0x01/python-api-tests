...
class RestClient:
    ...
    def _send_request(self, method, path, **kwargs):
        ...
        log.msg(event='request', method='method', host=full_url, params=kwargs.get('params'),
        ...
        print(curl)
        log.msg(event='response', status_code=response.status_code, headers=response.headers, json=self._get_json(response),
                text=response.text, content=response.content, curl=curl)
        return response
